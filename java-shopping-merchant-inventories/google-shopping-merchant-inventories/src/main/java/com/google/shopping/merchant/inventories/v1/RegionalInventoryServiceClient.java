...
import com.google.shopping.merchant.inventories.v1.stub.RegionalInventoryServiceStub;
 ...
 * <p>  Replaces the full `RegionalInventory` resource if an entry with the same [`region`][google.shopping.merchant.inventories.v1.RegionalInventory.region] already exists for the product.
...
public class RegionalInventoryServiceClient implements BackgroundResource {
   ...
   *     <p>1. &#42;&#42;Encoded Format&#42;&#42;: The `{product}` segment is an &#42;&#42;unpadded
   *     base64url&#42;&#42; encoded string (RFC 4648 Section 5). The decoded string must result in
   *     the `content_language~feed_label~offer_id` structure. This encoding MUST be used if any
   *     part of the product identifier (like `offer_id`) contains characters such as `/`, `%`, or
   ...
   *     full resource name for the product would be `accounts/123/products/ZW5-VVN-c2t1LzEyMw`.
   ...
   * [`region`][google.shopping.merchant.inventories.v1.RegionalInventory.region] already exists for
   * the product.
   *
   ...
   *     the `content_language~feed_label~offer_id` structure. This encoding MUST be used if any
   *     part of the product identifier (like `offer_id`) contains characters such as `/`, `%`, or
   *     `~`. &#42; Example: To represent the product ID `en~US~sku/123` for `region` "region123",
   *     the `{product}` segment must be the unpadded base64url encoding of this string, which is
   *     `ZW5-VVN-c2t1LzEyMw`. The full resource name for the regional inventory would be
