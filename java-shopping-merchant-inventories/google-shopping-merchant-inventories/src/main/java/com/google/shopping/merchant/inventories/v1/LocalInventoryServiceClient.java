...
import com.google.shopping.merchant.inventories.v1.stub.LocalInventoryServiceStub;
 ...
 * <p>  Replaces the full `LocalInventory` resource if an entry with the same [`storeCode`][google.shopping.merchant.inventories.v1.LocalInventory.store_code] already exists for the product.
...
public class LocalInventoryServiceClient implements BackgroundResource {
   ...
   *     <p>The `{product}` segment is a unique identifier for the product. This identifier must be
   ...
   *     `local~en~US~sku123`
   *     <p>The format of the `{product}` segment in the URL is automatically detected by the
   *     server, supporting two options:
   *     <p>1. &#42;&#42;Encoded Format&#42;&#42;: The `{product}` segment is an unpadded base64url
   ...
   * [`storeCode`][google.shopping.merchant.inventories.v1.LocalInventory.store_code] already exists
   * for the product.
   *
   ...
   *     `content_language~feed_label~offer_id` structure. This encoding MUST be used if any part of
   *     the product identifier (like `offer_id`) contains characters such as `/`, `%`, or `~`.
   *     &#42; Example: To represent the product ID `en~US~sku/123` for `store_code` "store123", the
   *     `{product}` segment must be the unpadded base64url encoding of this string, which is
   *     `ZW5-VVN-c2t1LzEyMw`. The full resource name for the local inventory would be
