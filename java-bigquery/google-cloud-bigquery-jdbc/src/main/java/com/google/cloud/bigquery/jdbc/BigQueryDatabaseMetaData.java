...
class BigQueryDatabaseMetaData implements DatabaseMetaData {
    ...
    Runnable procedureFetcher =
        () -> {
          ...
          try {
            ...
            LOG.info("Submitting parallel findMatchingRoutines tasks...");
            ...
            LOG.info("Finished submitting " + apiFutures.size() + " findMatchingRoutines tasks.");
            apiExecutor.shutdown();

            LOG.info("Processing results from findMatchingRoutines tasks...");
            ...
            if (Thread.currentThread().isInterrupted()) {
              ...
              LOG.info("Waiting for processProcedureInfo tasks to complete...");
              waitForTasksCompletion(processingTaskFutures);
              LOG.info("All processProcedureInfo tasks completed or handled.");
            }

            if (!Thread.currentThread().isInterrupted()) {
              Comparator<FieldValueList> comparator =
                  defineGetProceduresComparator(localResultSchemaFields);
              sortResults(collectedResults, comparator, "getProcedures");
            }
  ...
  void processProcedureInfo(
    ...
    LOG.info("Processing procedure info for: " + routineId);
    ...
    try {
      ...
      LOG.info("Processed and added procedure info row for: " + routineId);
        ...
        () -> {
          ...
          try {
            ...
            List<Routine> fullRoutines =
                fetchFullRoutineDetailsForIds(procedureIdsToGet, getRoutineDetailsExecutor);
            ...
            if (!Thread.currentThread().isInterrupted()) {
              ...
              sortResults(collectedResults, comparator, "getProcedureColumns");
  ...
  List<RoutineId> listMatchingProcedureIdsFromDatasets(
    ...
    for (Future<List<Routine>> listFuture : listRoutineFutures) {
      ...
      try {
        ...
        LOG.warning("Routine list task cancelled for catalog: " + catalogParam);
      }
    }
    LOG.info(
  ...
  List<Routine> fetchFullRoutineDetailsForIds(
      List<RoutineId> procedureIdsToGet, ExecutorService getRoutineDetailsExecutor)
      throws InterruptedException {
    LOG.finest("++enter++");
    LOG.info("Fetching full details for %d procedure IDs.", procedureIdsToGet.size());
    ...
    LOG.info("Submitted " + getRoutineFutures.size() + " getRoutine detail tasks.");
    ...
    for (Future<Routine> getFuture : getRoutineFutures) {
      ...
      try {
        ...
        LOG.warning("Error processing getRoutine future result: " + e.getCause());
      } catch (CancellationException e) {
        LOG.warning("getRoutine detail task cancelled.");
      }
    }
    LOG.info("Successfully fetched full details for %d routines.", fullRoutines.size());
    return fullRoutines;
  }
  ...
  void submitProcedureArgumentProcessingJobs(
      ...
      throws InterruptedException {
    LOG.info("Submitting argument processing jobs for %d routines.", fullRoutines.size());
    ...
    LOG.info(
        "Submitted %d procedure argument processing jobs.", outArgumentProcessingFutures.size());
  }
  ...
  void processProcedureArguments(
    ...
    if (arguments == null || arguments.isEmpty()) {
      LOG.info("Procedure " + routineId + " has no arguments.");
      return;
    }
    ...
    Runnable tableFetcher =
        () -> {
          ...
          try {
            ...
            LOG.info("Submitting parallel findMatchingTables tasks...");
            for (Dataset dataset : datasetsToScan) {
              if (Thread.currentThread().isInterrupted()) {
            ...
            LOG.info("Finished submitting " + apiFutures.size() + " findMatchingTables tasks.");
            apiExecutor.shutdown();

            LOG.info("Processing results from findMatchingTables tasks...");
            ...
            if (Thread.currentThread().isInterrupted()) {
              ...
              LOG.info("Waiting for processTableInfo tasks to complete...");
              waitForTasksCompletion(processingFutures);
              LOG.info("All processTableInfo tasks completed.");
            }

            if (!Thread.currentThread().isInterrupted()) {
              Comparator<FieldValueList> comparator =
                  defineGetTablesComparator(localResultSchemaFields);
              sortResults(collectedResults, comparator, "getTables");
            }
  ...
  void processTableInfo(
    ...
    LOG.info("Processing table info for: " + tableId);
    ...
    try {
      ...
      LOG.info("Processed and added table info row for: " + tableId);
    } catch (Exception e) {
      LOG.warning(
          ...
          try {
            ...
            for (Dataset dataset : datasetsToScan) {
              ...
              for (Table table : tablesToScan) {
                ...
                TableId tableId = table.getTableId();
                LOG.info("Submitting task for table: " + tableId);
            ...
            if (!Thread.currentThread().isInterrupted()) {
              ...
              sortResults(collectedResults, comparator, "getColumns");
            }
  ...
  private void processTableColumns(
    ...
    LOG.finest("++enter++");
    TableId tableId = table.getTableId();
    LOG.info("Processing columns for table: " + tableId);
    TableDefinition definition = table.getDefinition();
    ...
    try {
      ...
      LOG.info("Finished processing columns for table: " + tableId);
  ...
  private ResultSet getTypeInfoImpl() {
    ...
    sortResults(typeInfoRows, comparator, "getTypeInfo");
    final BlockingQueue<BigQueryFieldValueListWrapper> queue =
        ...
        () -> {
          ...
          try {
            for (String currentProjectToScan : projectsToScanList) {
              ...
              LOG.info("Processing found datasets for project: " + currentProjectToScan);
            ...
            if (!Thread.currentThread().isInterrupted()) {
              ...
              sortResults(collectedResults, comparator, "getSchemas");
            }
    ...
    try {
      ...
      Comparator<FieldValueList> comparator =
          Comparator.comparing(
              ...
              Comparator.nullsFirst(String::compareToIgnoreCase));
      sortResults(collectedResults, comparator, "getClientInfoProperties");
    ...
    Runnable functionFetcher =
        () -> {
          ...
          try {
            ...
            sortResults(collectedResults, comparator, "getFunctions");
            populateQueue(collectedResults, queue, localResultSchemaFields);
          } catch (Throwable t) {
  ...
  void processFunctionInfo(
      Routine routine, List<FieldValueList> collectedResults, FieldList resultSchemaFields) {
    RoutineId routineId = routine.getRoutineId();
    LOG.info("Processing function info for: " + routineId);
    ...
    try {
      ...
      FieldValueList rowFvl = FieldValueList.of(values, resultSchemaFields);
      collectedResults.add(rowFvl);
      LOG.info("Processed and added function info row for: " + routineId);
        ...
        () -> {
          ...
          try {
            ...
            List<Routine> fullFunctions =
                fetchFullRoutineDetailsForIds(functionIdsToGet, getRoutineDetailsExecutor);
            shutdownExecutor(getRoutineDetailsExecutor);
            getRoutineDetailsExecutor = null;
            ...
            if (!Thread.currentThread().isInterrupted()) {
              ...
              sortResults(collectedResults, comparator, "getFunctionColumns");
              populateQueue(collectedResults, queue, resultSchemaFields);
            }
  ...
  List<RoutineId> listMatchingFunctionIdsFromDatasets(
    ...
    for (Future<List<Routine>> listFuture : listRoutineFutures) {
      ...
      try {
        ...
        LOG.warning("Routine (function) list task cancelled for catalog: " + catalogParam);
      }
    }
    LOG.info(
  ...
  void submitFunctionParameterProcessingJobs(
      ...
      throws InterruptedException {
    LOG.info("Submitting parameter processing jobs for %d functions.", fullFunctions.size());

    for (Routine fullFunction : fullFunctions) {
      if (Thread.currentThread().isInterrupted()) {
        LOG.warning("Interrupted during submission of function parameter processing tasks.");
  ...
  <T> List<T> findMatchingBigQueryObjects(
    ...
    try {
      ...
      if (needsList) {
        ...
        LOG.info("Retrieved initial %s list, iterating & filtering if needed...", objectTypeName);
  ...
  void sortResults(
    ...
    if (collectedResults == null || collectedResults.isEmpty()) {
      LOG.info("No results collected for %s, skipping sort.", operationName);
      return;
    }
    if (comparator == null) {
      LOG.info("No comparator provided for %s, skipping sort.", operationName);
      return;
    }

    LOG.info("Sorting %d collected %s results...", collectedResults.size(), operationName);
    try {
      collectedResults.sort(comparator);
      LOG.info("%s result sorting completed.", operationName);
    } catch (Exception e) {
      LOG.severe("Error during sorting %s results: %s", operationName, e.getMessage());
    }
  }
  ...
  private List<String> getAccessibleCatalogNames() {
    ...
    LOG.info("Accessible catalogs: %s", sortedCatalogs);
