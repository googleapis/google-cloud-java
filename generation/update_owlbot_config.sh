...
# Usage:
# update_owlbot_config.sh <.OwlBot.yaml path from the root>
# Example:
# $ update_owlbot_config.sh java-dataform/.OwlBot.yaml
#
# To apply the change to all OwlBot configuration files in all modules:
# $ for F in `find . -maxdepth 2 -name '.OwlBot.yaml'`; do sh generation/update_owlbot_config.sh $F; done
