...
for module in $(find . -mindepth 1 -maxdepth 3 -name pom.xml | sort | xargs dirname); do
  ...
  if [[ "${groupId}" == *google.cloud* ]]; then
        ...
        sonatypeURL="https://google.oss.sonatype.org/content/repositories/comgooglecloud-5275/com/google/cloud/${artifactId}/${version}/${sonatypeJarFile}"
  ...
  if [[ "${groupId}" == *grpc* ]] ; then
        ...
        sonatypeURL="https://google.oss.sonatype.org/content/repositories/comgoogleapi-4854/com/google/api/grpc/${artifactId}/${version}/${sonatypeJarFile}"
