...
for module in $(find . -mindepth 2 -maxdepth 2 -name pom.xml |sort | xargs dirname); do
  ...
  sed -i.bak -e "/<parent>/{N;s|<groupId>.*<\/groupId>|<groupId>${parent_group_id}<\/groupId>|;N;s|<artifactId>.*<\/artifactId>|<artifactId>${parent_artifact_id}<\/artifactId>|;N;s|<version>.*<\/version>|<version>${parent_version}<\/version><!-- {x-version-update:google-cloud-java-parent:current} -->|;}" pom.xml && rm pom.xml.bak
  ...
  if ls | grep 'bom'; then
    ...
    sed -i.bak -e "/<parent>/{N;s|<groupId>.*<\/groupId>|<groupId>${parent_group_id}<\/groupId>|;N;s|<artifactId>.*<\/artifactId>|<artifactId>${parent_artifact_id}<\/artifactId>|;N;s|<version>.*<\/version>|<version>${parent_version}<\/version><!-- {x-version-update:google-cloud-java-parent:current} -->\n    <relativePath>..\/..\/pom.xml<\/relativePath>|;}" ${BOM}/pom.xml && rm ${BOM}/pom.xml.bak
  fi
