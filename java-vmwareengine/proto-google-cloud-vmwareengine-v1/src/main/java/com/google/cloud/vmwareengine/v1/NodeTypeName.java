...
public class NodeTypeName implements ResourceName {
  private static final PathTemplate PROJECT_LOCATION_NODE_TYPE =
  ...
  public static NodeTypeName of(String project, String location, String nodeType) {
    return newBuilder().setProject(project).setLocation(location).setNodeType(nodeType).build();
  }
  ...
  public static @Nullable NodeTypeName parse(String formattedString) {
    if (formattedString.isEmpty()) {
      return null;
    }
    Map<String, String> matchMap =
        PROJECT_LOCATION_NODE_TYPE.validatedMatch(
            formattedString, "NodeTypeName.parse: formattedString not in valid format");
    return of(matchMap.get("project"), matchMap.get("location"), matchMap.get("node_type"));
  }

  public static List<NodeTypeName> parseList(List<String> formattedStrings) {
    List<NodeTypeName> list = new ArrayList<>(formattedStrings.size());
  ...
  public static List<String> toStringList(List<@Nullable NodeTypeName> values) {
    ...
    if (fieldValuesMap == null) {
      synchronized (this) {
        if (fieldValuesMap == null) {
          ...
          if (nodeType != null) {
            fieldMapBuilder.put("node_type", nodeType);
          }
  ...
  public String toString() {
    return PROJECT_LOCATION_NODE_TYPE.instantiate(
        "project", project, "location", location, "node_type", nodeType);
  }
