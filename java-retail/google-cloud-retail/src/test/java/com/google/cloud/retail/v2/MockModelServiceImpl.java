...
import com.google.cloud.retail.v2.ModelServiceGrpc.ModelServiceImplBase;
  ...
  public void pauseModel(PauseModelRequest request, StreamObserver<Model> responseObserver) {
    ...
    if (response instanceof Model) {
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method PauseModel, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Model.class.getName(),
                  Exception.class.getName())));
    }
  ...
  public void resumeModel(ResumeModelRequest request, StreamObserver<Model> responseObserver) {
    ...
    if (response instanceof Model) {
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method ResumeModel, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Model.class.getName(),
                  Exception.class.getName())));
    }
  ...
  public void tuneModel(TuneModelRequest request, StreamObserver<Operation> responseObserver) {
    ...
    if (response instanceof Operation) {
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method TuneModel, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
    }
