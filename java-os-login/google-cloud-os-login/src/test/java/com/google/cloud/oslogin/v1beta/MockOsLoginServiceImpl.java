...
public class MockOsLoginServiceImpl extends OsLoginServiceImplBase {
  ...
  public void signSshPublicKey(
      SignSshPublicKeyRequest request, StreamObserver<SignSshPublicKeyResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof SignSshPublicKeyResponse) {
      requests.add(request);
      responseObserver.onNext(((SignSshPublicKeyResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method SignSshPublicKey, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  SignSshPublicKeyResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
}
