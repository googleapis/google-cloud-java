...
public class DocumentReferenceTest {
  ...
  public void deserializeBasicTypes() throws Exception {
    ...
    assertFalse(snapshot.contains("noValue"));
    assertEquals("default", snapshot.get("noValue", "default"));
    assertEquals("bar", snapshot.get("foo", "default"));
    assertTrue(snapshot.contains(FieldPath.of("foo")));
    assertEquals("bar", snapshot.getString("foo"));
    assertEquals("default", snapshot.getString("novalue", "default"));
    assertEquals("bar", snapshot.getString("foo", "default"));
    ...
    assertEquals((Double) 1.0, snapshot.getDouble("noValue", (Double) 1.0));
    assertEquals((Double) 0.0, snapshot.getDouble("doubleValue", (Double) 1.0));
    assertEquals((Long) 0L, snapshot.getLong("longValue"));
    assertEquals((Long) 1L, snapshot.getLong("noValue", 1L));
    assertEquals((Long) 0L, snapshot.getLong("longValue", 1L));
    assertTrue(snapshot.getBoolean("trueValue"));
    assertFalse(snapshot.getBoolean("noValue", false));
    assertTrue(snapshot.getBoolean("trueValue", false));
    assertEquals(DATE, snapshot.getDate("dateValue"));
    Date defaultDate = new Date(1234L);
    assertFalse(defaultDate.equals(DATE));
    assertEquals(defaultDate, snapshot.getDate("noValue", defaultDate));
    assertEquals(DATE, snapshot.getDate("dateValue", defaultDate));
    assertEquals(TIMESTAMP, snapshot.getTimestamp("timestampValue"));
    Timestamp defaultTimestamp = Timestamp.ofTimeSecondsAndNanos(1234, 0);
    assertFalse(defaultTimestamp.equals(TIMESTAMP));
    assertEquals(defaultTimestamp, snapshot.getTimestamp("noValue", defaultTimestamp));
    assertEquals(TIMESTAMP, snapshot.getTimestamp("timestampValue", defaultTimestamp));
    assertEquals(BLOB, snapshot.getBlob("bytesValue"));
    Blob defaultBlob = Blob.fromBytes(new byte[] {3,2,1});
    assertFalse(defaultBlob.equals(BLOB));
    assertEquals(defaultBlob, snapshot.getBlob("noValue", defaultBlob));
    assertEquals(BLOB, snapshot.getBlob("bytesValue", defaultBlob));
    ...
    GeoPoint defaultGeopoint = new GeoPoint(30L, 30);
    assertEquals(defaultGeopoint, snapshot.getGeoPoint("noValue", defaultGeopoint));
    assertEquals(GEO_POINT, snapshot.getGeoPoint("geoPointValue", defaultGeopoint));
