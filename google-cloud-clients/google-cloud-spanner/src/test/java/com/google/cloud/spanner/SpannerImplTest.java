...
public class SpannerImplTest {
  ...
  @Test
  public void testConnectionMaxAttempt() {
    Map<String, String> labels = new HashMap<>();
    labels.put("env", "dev");
    Mockito.when(spannerOptions.getSessionLabels()).thenReturn(labels);
    String dbName = "projects/p1/instances/i1/databases/d1";
    ...
    Mockito.when(rpc.createSession(Mockito.eq(dbName), Mockito.eq(labels), options.capture()))
        .thenThrow(
            SpannerExceptionFactory.newSpannerException(
                ErrorCode.UNAVAILABLE, "Unable to resolve host spanner.googleapis.com"));
    try {
      impl.getDatabaseClient(db).singleUse();
    } catch (SpannerException e) {
      assertThat(e.getErrorCode()).isEqualTo(ErrorCode.INTERNAL);
      assertThat(e.getMessage()).contains("Exceeded maxAttempt");
      impl.close();
    }
  }

  @Test
  public void testConnectionTotalTimeout() {
    Map<String, String> labels = new HashMap<>();
    labels.put("env", "dev");
    String dbName = "projects/p1/instances/i1/databases/d1";
    DatabaseId db = DatabaseId.of(dbName);
    RetrySettings retrySettings =
        RetrySettings.newBuilder()
            .setTotalTimeout(Duration.ofSeconds(4))
            .setMaxAttempts(10)
            .build();
    Mockito.when(spannerOptions.getRetrySettings()).thenReturn(retrySettings);
    Mockito.when(spannerOptions.getSessionLabels()).thenReturn(labels);
    Mockito.when(spannerOptions.getTransportOptions())
        .thenReturn(GrpcTransportOptions.newBuilder().build());
    Mockito.when(spannerOptions.getSessionPoolOptions())
    ...
    try {
      ...
      assertThat(e.getErrorCode()).isEqualTo(ErrorCode.INTERNAL);
      assertThat(e.getMessage()).contains("Exceeded totalTimeout");
      impl.close();
    }
  }
}
