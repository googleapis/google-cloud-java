 ...
 * <code>mvn -P generate-test-sql-scripts compile</code>
 *
 * It is only necessary to generate a new test script if the behavior of {@link Connection} has
 * changed (for example calling COMMIT is currently not allowed in AUTOCOMMIT mode, but this has
 * changed to be a no-op). A new test script must also be generated if additional test cases have
 * been added to {@link AbstractConnectionImplTest}.
 */
...
public class ConnectionImplGeneratedSqlScriptTest {
  ...
  private static Class<?>[] getAbstractConnectionImplTestSubclasses()
    ...
    ImmutableSet<ClassInfo> classes = cp.getTopLevelClassesRecursive(
  ...
  private static void addAbstractConnectionImplTestSubclassesToList(List<Class<?>> list,
      Class<?> clazz) {
