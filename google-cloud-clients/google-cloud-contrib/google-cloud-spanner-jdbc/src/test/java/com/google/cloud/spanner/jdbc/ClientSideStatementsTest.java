 ...
 * generated by running <code>mvn -P generate-test-sql-scripts compile</code>
 *
 * It is only necessary to generate a new test script if a new {@link ClientSideStatement} has been
 * added, or the behavior of an existing {@link ClientSideStatement} has changed.
 *
...
public class ClientSideStatementsTest {
  ...
  private static void generateTestStatements(ClientSideStatementImpl statement) {
    for (String sql : statement.getExampleStatements()) {
      ...
      log(statement.getExamplePrerequisiteStatements(), withInvalidPrefix(sql),
          ErrorCode.INVALID_ARGUMENT);
      log(statement.getExamplePrerequisiteStatements(), withInvalidSuffix(sql),
          ErrorCode.INVALID_ARGUMENT);

      final String[] replacements = {"%", "_", "&", "$", "@", "!", "*", "(", ")", "-", "+", "-#",
          "/", "\\", "?", "-/", "/#", "/-"};
      for (String replacement : replacements) {
        log(statement.getExamplePrerequisiteStatements(), withPrefix(replacement, sql),
            ErrorCode.INVALID_ARGUMENT);
        log(statement.getExamplePrerequisiteStatements(), withSuffix(replacement, sql),
            ErrorCode.INVALID_ARGUMENT);
        log(statement.getExamplePrerequisiteStatements(), replaceLastSpaceWith(replacement, sql),
            ErrorCode.INVALID_ARGUMENT);
      }
    }
  }
  ...
  private static String replaceLastSpaceWith(String replacement, String statement) {
    if (statement.lastIndexOf(' ') > -1) {
      return statement.substring(0, statement.lastIndexOf(' ')) + replacement
