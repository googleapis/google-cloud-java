...
public final class CloudStorageFileSystemProvider extends FileSystemProvider {
  ...
  public boolean deleteIfExists(Path path) throws IOException {
    ...
    final CloudStorageRetryHandler retryHandler =
        new CloudStorageRetryHandler(CloudStorageUtil.getMaxChannelReopensFromPath(path));
  ...
  public void copy(Path source, Path target, CopyOption... options) throws IOException {
    ...
    final CloudStorageRetryHandler retryHandler =
        new CloudStorageRetryHandler(CloudStorageUtil.getMaxChannelReopensFromPath(source));
    // Loop will terminate via an exception if all retries are exhausted
    while (true) {
      try {
  ...
  public DirectoryStream<Path> newDirectoryStream(
    ...
    final CloudStorageRetryHandler retryHandler =
        new CloudStorageRetryHandler(CloudStorageUtil.getMaxChannelReopensFromPath(dir));
    // Loop will terminate via an exception if all retries are exhausted
    while (true) {
      try {
