...
import com.google.cloud.conformance.bigtable.v2.TestDefinition.ReadRowsTest.Result;
...
public class ReadRowsMergingAcceptanceTest {
  ...
  public static Collection<Object[]> data() throws IOException {
    ...
    final TestFile testDefinition = testBuilder.build();

    final List<ReadRowsTest> tests = testDefinition.getReadRowsTestsList();
    final ArrayList<Object[]> data = new ArrayList<>(tests.size());
    for (ReadRowsTest test : tests) {
      final String junitName =
  ...
  public void test() throws Exception {
    ...
    List<Result> actualResults = Lists.newArrayList();
  ...
  private static boolean expectsError(ReadRowsTest rrt) {
    final List<Result> results = rrt.getResultsList();
    return results != null && !results.isEmpty() && results.get(results.size() - 1).getError();
  }

  private static List<Result> getNonExceptionResults(ReadRowsTest rrt) {
    final List<Result> results = rrt.getResultsList();
