...
public class RuleSetServiceClient implements BackgroundResource {
   ...
   * try (RuleSetServiceClient ruleSetServiceClient = RuleSetServiceClient.create()) {
   *   RuleSet ruleSet = RuleSet.newBuilder().build();
   *   RuleSet response = ruleSetServiceClient.updateRuleSet(ruleSet);
   * }
   * }</pre>
   *
   ...
   * @throws com.google.api.gax.rpc.ApiException if the remote call fails
   */
  public final RuleSet updateRuleSet(RuleSet ruleSet) {
    UpdateRuleSetRequest request = UpdateRuleSetRequest.newBuilder().setRuleSet(ruleSet).build();
