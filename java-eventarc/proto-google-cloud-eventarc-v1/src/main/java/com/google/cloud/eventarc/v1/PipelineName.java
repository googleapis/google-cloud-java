...
public class PipelineName implements ResourceName {
  private static final PathTemplate PROJECT_LOCATION_PIPELINE =
  ...
  private PipelineName(Builder builder) {
    ...
    pipeline = Preconditions.checkNotNull(builder.getPipeline());
  }
  ...
  public static PipelineName of(String project, String location, String pipeline) {
    return newBuilder().setProject(project).setLocation(location).setPipeline(pipeline).build();
  }
  ...
  public static @Nullable PipelineName parse(String formattedString) {
    if (formattedString.isEmpty()) {
      return null;
    }
    Map<String, String> matchMap =
        PROJECT_LOCATION_PIPELINE.validatedMatch(
            formattedString, "PipelineName.parse: formattedString not in valid format");
    return of(matchMap.get("project"), matchMap.get("location"), matchMap.get("pipeline"));
  }

  public static List<PipelineName> parseList(List<String> formattedStrings) {
    List<PipelineName> list = new ArrayList<>(formattedStrings.size());
  ...
  public static List<String> toStringList(List<@Nullable PipelineName> values) {
    ...
    if (fieldValuesMap == null) {
      synchronized (this) {
        if (fieldValuesMap == null) {
          ...
          if (pipeline != null) {
            fieldMapBuilder.put("pipeline", pipeline);
          }
  ...
  public String toString() {
    return PROJECT_LOCATION_PIPELINE.instantiate(
        "project", project, "location", location, "pipeline", pipeline);
  }
