...
import com.google.cloud.storage.ITRcuBidiWriteTest.UploadConfigParameters;
...
public final class ITRcuBidiWriteTest {
  ...
  public void appendableBlobUploadTakeover() throws Exception {
    ...
    BlobAppendableUpload upload =
        storage.blobAppendableUpload(
            BlobInfo.newBuilder(id).setStorageClass(StorageClass.valueOf("RAPID")).build(),
            doNotFinalizeConfig);
    try (AppendableUploadWriteableByteChannel channel = upload.open()) {
  ...
  public void testUploadFileUsingAppendable() throws Exception {
    ...
    try (TmpFile tmpFile =
      ...
      BlobAppendableUpload appendable =
          storage.blobAppendableUpload(
              BlobInfo.newBuilder(bid).setStorageClass(StorageClass.valueOf("RAPID")).build(),
              p.uploadConfig);
