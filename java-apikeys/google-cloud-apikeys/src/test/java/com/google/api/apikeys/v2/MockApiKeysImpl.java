...
import com.google.api.apikeys.v2.ApiKeysGrpc.ApiKeysImplBase;
...
public class MockApiKeysImpl extends ApiKeysImplBase {
  private List<AbstractMessage> requests;
  private Queue<Object> responses;

  public MockApiKeysImpl() {
  ...
  public void getKeyString(
      GetKeyStringRequest request, StreamObserver<GetKeyStringResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof GetKeyStringResponse) {
      requests.add(request);
      responseObserver.onNext(((GetKeyStringResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method GetKeyString, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  GetKeyStringResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
  ...
  public void undeleteKey(UndeleteKeyRequest request, StreamObserver<Operation> responseObserver) {
    ...
    if (response instanceof Operation) {
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method UndeleteKey, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
    }
  ...
  public void lookupKey(
      LookupKeyRequest request, StreamObserver<LookupKeyResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof LookupKeyResponse) {
      requests.add(request);
      responseObserver.onNext(((LookupKeyResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method LookupKey, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  LookupKeyResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
}
