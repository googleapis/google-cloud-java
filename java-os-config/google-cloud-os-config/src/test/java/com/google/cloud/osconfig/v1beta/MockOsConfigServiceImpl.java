...
import com.google.cloud.osconfig.v1beta.OsConfigServiceGrpc.OsConfigServiceImplBase;
...
public class MockOsConfigServiceImpl extends OsConfigServiceImplBase {
  ...
  public void createGuestPolicy(
    ...
    Object response = responses.poll();
    if (response instanceof GuestPolicies.GuestPolicy) {
      requests.add(request);
      responseObserver.onNext(((GuestPolicies.GuestPolicy) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
  ...
  public void listGuestPolicies(
      ...
      StreamObserver<GuestPolicies.ListGuestPoliciesResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof GuestPolicies.ListGuestPoliciesResponse) {
      requests.add(request);
      responseObserver.onNext(((GuestPolicies.ListGuestPoliciesResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
  ...
  public void deleteGuestPolicy(
      GuestPolicies.DeleteGuestPolicyRequest request, StreamObserver<Empty> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Empty) {
      requests.add(request);
      responseObserver.onNext(((Empty) response));
