...
public class ParticipantsClientTest {
  ...
  @Test
  public void listSuggestionsTest() throws Exception {
    Suggestion responsesElement = Suggestion.newBuilder().build();
    ListSuggestionsResponse expectedResponse =
        ListSuggestionsResponse.newBuilder()
            .setNextPageToken("")
            .addAllSuggestions(Arrays.asList(responsesElement))
            .build();
    mockParticipants.addResponse(expectedResponse);
    ...
    ListSuggestionsPagedResponse pagedListResponse = client.listSuggestions(request);

    List<Suggestion> resources = Lists.newArrayList(pagedListResponse.iterateAll());

    Assert.assertEquals(1, resources.size());
    Assert.assertEquals(expectedResponse.getSuggestionsList().get(0), resources.get(0));

    List<AbstractMessage> actualRequests = mockParticipants.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    ListSuggestionsRequest actualRequest = ((ListSuggestionsRequest) actualRequests.get(0));
  ...
  @Test
  public void listSuggestionsExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockParticipants.addException(exception);
    ...
    try {
      ListSuggestionsRequest request =
          ListSuggestionsRequest.newBuilder()
              ...
              .setFilter("filter-1274492040")
              .build();
      client.listSuggestions(request);
      Assert.fail("No exception raised");
    } catch (InvalidArgumentException e) {
      // Expected exception.
    }
  ...
  @Test
  public void compileSuggestionTest() throws Exception {
    CompileSuggestionResponse expectedResponse =
        CompileSuggestionResponse.newBuilder()
            .setSuggestion(Suggestion.newBuilder().build())
            .setLatestMessage("latestMessage-1424305536")
            .setContextSize(1116903569)
            .build();
    mockParticipants.addResponse(expectedResponse);
    ...
    CompileSuggestionResponse actualResponse = client.compileSuggestion(request);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<AbstractMessage> actualRequests = mockParticipants.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    CompileSuggestionRequest actualRequest = ((CompileSuggestionRequest) actualRequests.get(0));
  ...
  @Test
  public void compileSuggestionExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockParticipants.addException(exception);
