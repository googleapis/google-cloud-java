...
import com.google.cloud.dialogflow.v2.DocumentsGrpc.DocumentsImplBase;
...
public class MockDocumentsImpl extends DocumentsImplBase {
  ...
  public void exportDocument(
      ExportDocumentRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method ExportDocument, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
    }
