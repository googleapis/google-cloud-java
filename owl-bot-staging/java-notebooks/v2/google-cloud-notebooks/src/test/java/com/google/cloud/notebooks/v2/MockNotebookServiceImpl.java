...
import com.google.cloud.notebooks.v2.NotebookServiceGrpc.NotebookServiceImplBase;
...
public class MockNotebookServiceImpl extends NotebookServiceImplBase {
  ...
  @Override
  public void checkInstanceUpgradability(
      CheckInstanceUpgradabilityRequest request,
      StreamObserver<CheckInstanceUpgradabilityResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof CheckInstanceUpgradabilityResponse) {
      requests.add(request);
      responseObserver.onNext(((CheckInstanceUpgradabilityResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
