...
import com.google.cloud.dialogflow.v2beta1.ParticipantsGrpc.ParticipantsImplBase;
...
public class MockParticipantsImpl extends ParticipantsImplBase {
  ...
  public void listSuggestions(
    ...
    Object response = responses.poll();
    if (response instanceof ListSuggestionsResponse) {
      requests.add(request);
      responseObserver.onNext(((ListSuggestionsResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method ListSuggestions, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  ListSuggestionsResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
  ...
  public void compileSuggestion(
    ...
    Object response = responses.poll();
    if (response instanceof CompileSuggestionResponse) {
      requests.add(request);
      responseObserver.onNext(((CompileSuggestionResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method CompileSuggestion, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  CompileSuggestionResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
}
