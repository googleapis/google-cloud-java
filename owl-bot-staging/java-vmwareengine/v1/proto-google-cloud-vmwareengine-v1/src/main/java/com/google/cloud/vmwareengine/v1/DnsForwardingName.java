...
public class DnsForwardingName implements ResourceName {
  ...
  @Deprecated
  protected DnsForwardingName() {
    project = null;
    location = null;
    privateCloud = null;
  }

  private DnsForwardingName(Builder builder) {
  ...
  public static DnsForwardingName of(String project, String location, String privateCloud) {
    return newBuilder()
        .setProject(project)
  ...
  public static DnsForwardingName parse(String formattedString) {
    if (formattedString.isEmpty()) {
      return null;
    }
    Map<String, String> matchMap =
        PROJECT_LOCATION_PRIVATE_CLOUD.validatedMatch(
            formattedString, "DnsForwardingName.parse: formattedString not in valid format");
    return of(matchMap.get("project"), matchMap.get("location"), matchMap.get("private_cloud"));
  }

  public static List<DnsForwardingName> parseList(List<String> formattedStrings) {
    List<DnsForwardingName> list = new ArrayList<>(formattedStrings.size());
  ...
  public static List<String> toStringList(List<DnsForwardingName> values) {
    List<String> list = new ArrayList<>(values.size());
    for (DnsForwardingName value : values) {
      if (value == null) {
        list.add("");
      } else {
  ...
  public boolean equals(Object o) {
    ...
    if (o != null && getClass() == o.getClass()) {
      DnsForwardingName that = ((DnsForwardingName) o);
  ...
  public static class Builder {
    ...
    private Builder(DnsForwardingName dnsForwardingName) {
      this.project = dnsForwardingName.project;
      this.location = dnsForwardingName.location;
      this.privateCloud = dnsForwardingName.privateCloud;
    }

    public DnsForwardingName build() {
      return new DnsForwardingName(this);
    }
  }
}
