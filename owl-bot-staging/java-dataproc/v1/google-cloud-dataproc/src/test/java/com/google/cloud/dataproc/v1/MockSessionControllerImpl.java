...
import com.google.cloud.dataproc.v1.SessionControllerGrpc.SessionControllerImplBase;
...
public class MockSessionControllerImpl extends SessionControllerImplBase {
  private List<AbstractMessage> requests;
  private Queue<Object> responses;

  public MockSessionControllerImpl() {
  ...
  public void createSession(
      CreateSessionRequest request, StreamObserver<Operation> responseObserver) {
  ...
  public void terminateSession(
      TerminateSessionRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method TerminateSession, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
    }
  ...
  public void deleteSession(
      DeleteSessionRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
