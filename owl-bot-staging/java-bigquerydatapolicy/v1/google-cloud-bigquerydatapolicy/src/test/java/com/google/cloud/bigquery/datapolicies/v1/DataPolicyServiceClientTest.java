...
public class DataPolicyServiceClientTest {
  ...
  public void renameDataPolicyTest() throws Exception {
    ...
    List<AbstractMessage> actualRequests = mockDataPolicyService.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    RenameDataPolicyRequest actualRequest = ((RenameDataPolicyRequest) actualRequests.get(0));

    Assert.assertEquals(name, actualRequest.getName());
    Assert.assertEquals(newDataPolicyId, actualRequest.getNewDataPolicyId());
    Assert.assertTrue(
        channelProvider.isHeaderSent(
            ApiClientHeaderProvider.getDefaultApiClientHeaderKey(),
            GaxGrpcProperties.getDefaultApiClientHeaderPattern()));
