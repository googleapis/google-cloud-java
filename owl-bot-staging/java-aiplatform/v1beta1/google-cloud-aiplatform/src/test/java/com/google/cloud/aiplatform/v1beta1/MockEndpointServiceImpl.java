...
import com.google.cloud.aiplatform.v1beta1.EndpointServiceGrpc.EndpointServiceImplBase;
...
public class MockEndpointServiceImpl extends EndpointServiceImplBase {
  private List<AbstractMessage> requests;
  private Queue<Object> responses;

  public MockEndpointServiceImpl() {
  ...
  public void mutateDeployedModel(
      MutateDeployedModelRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method MutateDeployedModel, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
    }
