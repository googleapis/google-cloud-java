...
import com.google.cloud.recommendationengine.v1beta1.UserEventServiceGrpc.UserEventServiceImplBase;
...
public class MockUserEventServiceImpl extends UserEventServiceImplBase {
  ...
  public void listUserEvents(
      ListUserEventsRequest request, StreamObserver<ListUserEventsResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof ListUserEventsResponse) {
      requests.add(request);
      responseObserver.onNext(((ListUserEventsResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method ListUserEvents, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  ListUserEventsResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
