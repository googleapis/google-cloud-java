...
public class ChallengeName implements ResourceName {
  private static final PathTemplate PROJECT_LOCATION_UUID =
  ...
  private ChallengeName(Builder builder) {
    ...
    uuid = Preconditions.checkNotNull(builder.getUuid());
  }
  ...
  public static ChallengeName of(String project, String location, String uuid) {
    return newBuilder().setProject(project).setLocation(location).setUuid(uuid).build();
  }

  public static String format(String project, String location, String uuid) {
    return newBuilder().setProject(project).setLocation(location).setUuid(uuid).build().toString();
  }

  public static @Nullable ChallengeName parse(String formattedString) {
    if (formattedString.isEmpty()) {
      return null;
    }
    Map<String, String> matchMap =
        PROJECT_LOCATION_UUID.validatedMatch(
            formattedString, "ChallengeName.parse: formattedString not in valid format");
    return of(matchMap.get("project"), matchMap.get("location"), matchMap.get("uuid"));
  }

  public static List<ChallengeName> parseList(List<String> formattedStrings) {
    List<ChallengeName> list = new ArrayList<>(formattedStrings.size());
  ...
  public static List<String> toStringList(List<@Nullable ChallengeName> values) {
  ...
  public String toString() {
    return PROJECT_LOCATION_UUID.instantiate(
        "project", project, "location", location, "uuid", uuid);
  }
