...
import com.google.cloud.chronicle.v1.BigQueryExportServiceGrpc.BigQueryExportServiceImplBase;
...
public class MockBigQueryExportServiceImpl extends BigQueryExportServiceImplBase {
  private List<AbstractMessage> requests;
  private Queue<Object> responses;

  public MockBigQueryExportServiceImpl() {
  ...
  public void provisionBigQueryExport(
      ProvisionBigQueryExportRequest request, StreamObserver<BigQueryExport> responseObserver) {
    Object response = responses.poll();
    if (response instanceof BigQueryExport) {
      requests.add(request);
      responseObserver.onNext(((BigQueryExport) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method ProvisionBigQueryExport, expected %s or"
                      + " %s",
                  response == null ? "null" : response.getClass().getName(),
                  BigQueryExport.class.getName(),
                  Exception.class.getName())));
