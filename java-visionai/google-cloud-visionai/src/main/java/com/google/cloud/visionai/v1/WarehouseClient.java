...
public class WarehouseClient implements BackgroundResource {
   ...
   *           .setPageToken("pageToken873572522")
   *           .build();
   *   for (Location element : warehouseClient.listLocations(request).iterateAll()) {
   *     // doThingsWith(element);
   *   }
   * }
   * }</pre>
   ...
   *           .setPageToken("pageToken873572522")
   *           .build();
   *   ApiFuture<Location> future = warehouseClient.listLocationsPagedCallable().futureCall(request);
   *   // Do something.
   ...
   *           .build();
   *   while (true) {
   *     ListLocationsResponse response = warehouseClient.listLocationsCallable().call(request);
   ...
   * try (WarehouseClient warehouseClient = WarehouseClient.create()) {
   *   GetLocationRequest request = GetLocationRequest.newBuilder().setName("name3373707").build();
   *   ApiFuture<Location> future = warehouseClient.getLocationCallable().futureCall(request);
   *   // Do something.
