...
public class AppPlatformClient implements BackgroundResource {
   ...
   *           .setPageToken("pageToken873572522")
   *           .build();
   *   for (Location element : appPlatformClient.listLocations(request).iterateAll()) {
   *     // doThingsWith(element);
   *   }
   * }
   * }</pre>
   ...
   *       appPlatformClient.listLocationsPagedCallable().futureCall(request);
   *   // Do something.
   ...
   * try (AppPlatformClient appPlatformClient = AppPlatformClient.create()) {
   *   GetLocationRequest request = GetLocationRequest.newBuilder().setName("name3373707").build();
   *   ApiFuture<Location> future = appPlatformClient.getLocationCallable().futureCall(request);
   *   // Do something.
