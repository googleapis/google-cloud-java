...
import com.google.api.serviceusage.v1.ServiceUsageGrpc.ServiceUsageImplBase;
...
public class MockServiceUsageImpl extends ServiceUsageImplBase {
  ...
  public void batchGetServices(
      BatchGetServicesRequest request, StreamObserver<BatchGetServicesResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof BatchGetServicesResponse) {
      requests.add(request);
      responseObserver.onNext(((BatchGetServicesResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method BatchGetServices, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
