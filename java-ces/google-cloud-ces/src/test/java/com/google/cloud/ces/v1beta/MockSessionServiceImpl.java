...
import com.google.cloud.ces.v1beta.SessionServiceGrpc.SessionServiceImplBase;
...
public class MockSessionServiceImpl extends SessionServiceImplBase {
    ...
    if (response instanceof RunSessionResponse) {
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method RunSession, expected %s or %s",
  ...
  public StreamObserver<BidiSessionClientMessage> bidiRunSession(
    ...
    StreamObserver<BidiSessionClientMessage> requestObserver =
        new StreamObserver<BidiSessionClientMessage>() {
          @Override
          public void onNext(BidiSessionClientMessage value) {
            requests.add(value);
            final Object response = responses.remove();
            if (response instanceof BidiSessionServerMessage) {
              responseObserver.onNext(((BidiSessionServerMessage) response));
            } else if (response instanceof Exception) {
              responseObserver.onError(((Exception) response));
            } else {
              responseObserver.onError(
                  new IllegalArgumentException(
                      String.format(
                          "Unrecognized response type %s for method BidiRunSession, expected %s or"
                              + " %s",
                          response == null ? "null" : response.getClass().getName(),
                          BidiSessionServerMessage.class.getName(),
                          Exception.class.getName())));
            }
          }
