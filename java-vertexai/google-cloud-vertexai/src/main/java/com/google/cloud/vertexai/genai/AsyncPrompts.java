...
public final class AsyncPrompts {
  ...
  CompletableFuture<CustomJob> privateCreateCustomJobResource(
    ...
    BuiltRequest builtRequest =
        prompts.buildRequestForPrivateCreateCustomJobResource(customJob, config);
    return this.apiClient
        .asyncRequest("post", builtRequest.path(), builtRequest.body(), builtRequest.httpOptions())
        .thenApplyAsync(
            response -> {
              try (ApiResponse res = response) {
                return prompts.processResponseForPrivateCreateCustomJobResource(res, config);
              }
            });
  }
  ...
  CompletableFuture<CustomJob> privateGetCustomJob(String name, VertexBaseConfig config) {
    ...
    BuiltRequest builtRequest = prompts.buildRequestForPrivateGetCustomJob(name, config);
    return this.apiClient
        .asyncRequest("get", builtRequest.path(), builtRequest.body(), builtRequest.httpOptions())
        .thenApplyAsync(
            response -> {
              try (ApiResponse res = response) {
                return prompts.processResponseForPrivateGetCustomJob(res, config);
              }
            });
  }

  CompletableFuture<OptimizeResponseEndpoint> privateOptimize(
      Content content, OptimizeConfig config) {

    BuiltRequest builtRequest = prompts.buildRequestForPrivateOptimize(content, config);
    return this.apiClient
        .asyncRequest("post", builtRequest.path(), builtRequest.body(), builtRequest.httpOptions())
        .thenApplyAsync(
            response -> {
              try (ApiResponse res = response) {
                return prompts.processResponseForPrivateOptimize(res, config);
              }
            });
  }
}
