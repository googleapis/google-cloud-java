...
public final class Prompts {
  ...
  /** A shared buildRequest method for both sync and async methods. */
  BuiltRequest buildRequestForPrivateOptimize(Content content, OptimizeConfig config) {
  ...
  /** A shared processResponse function for both sync and async methods. */
  OptimizeResponseEndpoint processResponseForPrivateOptimize(
  ...
  OptimizeResponseEndpoint privateOptimize(Content content, OptimizeConfig config) {
    BuiltRequest builtRequest = buildRequestForPrivateOptimize(content, config);

    try (ApiResponse response =
        this.apiClient.request(
            "post", builtRequest.path(), builtRequest.body(), builtRequest.httpOptions())) {
      return processResponseForPrivateOptimize(response, config);
    }
  }
}
