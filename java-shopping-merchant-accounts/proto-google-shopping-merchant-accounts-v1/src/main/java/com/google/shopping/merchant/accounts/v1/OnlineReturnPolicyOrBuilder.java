/*
 * Copyright 2025 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: google/shopping/merchant/accounts/v1/online_return_policy.proto

// Protobuf Java Version: 3.25.8
package com.google.shopping.merchant.accounts.v1;

public interface OnlineReturnPolicyOrBuilder
    extends
    // @@protoc_insertion_point(interface_extends:google.shopping.merchant.accounts.v1.OnlineReturnPolicy)
    com.google.protobuf.MessageOrBuilder {

  /**
   *
   *
   * <pre>
   * Identifier. The name of the `OnlineReturnPolicy` resource.
   * Format: `accounts/{account}/onlineReturnPolicies/{return_policy}`
   * </pre>
   *
   * <code>string name = 1 [(.google.api.field_behavior) = IDENTIFIER];</code>
   *
   * @return The name.
   */
  java.lang.String getName();

  /**
   *
   *
   * <pre>
   * Identifier. The name of the `OnlineReturnPolicy` resource.
   * Format: `accounts/{account}/onlineReturnPolicies/{return_policy}`
   * </pre>
   *
   * <code>string name = 1 [(.google.api.field_behavior) = IDENTIFIER];</code>
   *
   * @return The bytes for name.
   */
  com.google.protobuf.ByteString getNameBytes();

  /**
   *
   *
   * <pre>
   * Output only. Return policy ID generated by Google.
   * </pre>
   *
   * <code>string return_policy_id = 2 [(.google.api.field_behavior) = OUTPUT_ONLY];</code>
   *
   * @return The returnPolicyId.
   */
  java.lang.String getReturnPolicyId();

  /**
   *
   *
   * <pre>
   * Output only. Return policy ID generated by Google.
   * </pre>
   *
   * <code>string return_policy_id = 2 [(.google.api.field_behavior) = OUTPUT_ONLY];</code>
   *
   * @return The bytes for returnPolicyId.
   */
  com.google.protobuf.ByteString getReturnPolicyIdBytes();

  /**
   *
   *
   * <pre>
   * Optional. Immutable. This field represents the unique user-defined label of
   * the return policy for the given country. It is important to note that the
   * same label cannot be used in different return policies for the same
   * country. If not given, policies will be automatically treated as the
   * 'default' for the country. When using label, you are creating an exception
   * policy in that country to assign a custom return policy to certain product
   * groups, follow the instructions provided in the [Return policy label]
   * (https://support.google.com/merchants/answer/9445425). The label can
   * contain up to 50 characters.
   * </pre>
   *
   * <code>
   * string label = 3 [(.google.api.field_behavior) = IMMUTABLE, (.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The label.
   */
  java.lang.String getLabel();

  /**
   *
   *
   * <pre>
   * Optional. Immutable. This field represents the unique user-defined label of
   * the return policy for the given country. It is important to note that the
   * same label cannot be used in different return policies for the same
   * country. If not given, policies will be automatically treated as the
   * 'default' for the country. When using label, you are creating an exception
   * policy in that country to assign a custom return policy to certain product
   * groups, follow the instructions provided in the [Return policy label]
   * (https://support.google.com/merchants/answer/9445425). The label can
   * contain up to 50 characters.
   * </pre>
   *
   * <code>
   * string label = 3 [(.google.api.field_behavior) = IMMUTABLE, (.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The bytes for label.
   */
  com.google.protobuf.ByteString getLabelBytes();

  /**
   *
   *
   * <pre>
   * Required. Immutable. The countries of sale where the return policy applies.
   * The values must be a valid 2 letter ISO 3166 code.
   * </pre>
   *
   * <code>
   * repeated string countries = 4 [(.google.api.field_behavior) = REQUIRED, (.google.api.field_behavior) = IMMUTABLE];
   * </code>
   *
   * @return A list containing the countries.
   */
  java.util.List<java.lang.String> getCountriesList();

  /**
   *
   *
   * <pre>
   * Required. Immutable. The countries of sale where the return policy applies.
   * The values must be a valid 2 letter ISO 3166 code.
   * </pre>
   *
   * <code>
   * repeated string countries = 4 [(.google.api.field_behavior) = REQUIRED, (.google.api.field_behavior) = IMMUTABLE];
   * </code>
   *
   * @return The count of countries.
   */
  int getCountriesCount();

  /**
   *
   *
   * <pre>
   * Required. Immutable. The countries of sale where the return policy applies.
   * The values must be a valid 2 letter ISO 3166 code.
   * </pre>
   *
   * <code>
   * repeated string countries = 4 [(.google.api.field_behavior) = REQUIRED, (.google.api.field_behavior) = IMMUTABLE];
   * </code>
   *
   * @param index The index of the element to return.
   * @return The countries at the given index.
   */
  java.lang.String getCountries(int index);

  /**
   *
   *
   * <pre>
   * Required. Immutable. The countries of sale where the return policy applies.
   * The values must be a valid 2 letter ISO 3166 code.
   * </pre>
   *
   * <code>
   * repeated string countries = 4 [(.google.api.field_behavior) = REQUIRED, (.google.api.field_behavior) = IMMUTABLE];
   * </code>
   *
   * @param index The index of the value to return.
   * @return The bytes of the countries at the given index.
   */
  com.google.protobuf.ByteString getCountriesBytes(int index);

  /**
   *
   *
   * <pre>
   * Optional. The return policy.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.Policy policy = 5 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return Whether the policy field is set.
   */
  boolean hasPolicy();

  /**
   *
   *
   * <pre>
   * Optional. The return policy.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.Policy policy = 5 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The policy.
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.Policy getPolicy();

  /**
   *
   *
   * <pre>
   * Optional. The return policy.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.Policy policy = 5 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.PolicyOrBuilder getPolicyOrBuilder();

  /**
   *
   *
   * <pre>
   * Optional. Overrides to the general policy for orders placed during a
   * specific set of time intervals.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverride seasonal_overrides = 14 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  java.util.List<com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverride>
      getSeasonalOverridesList();

  /**
   *
   *
   * <pre>
   * Optional. Overrides to the general policy for orders placed during a
   * specific set of time intervals.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverride seasonal_overrides = 14 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverride getSeasonalOverrides(
      int index);

  /**
   *
   *
   * <pre>
   * Optional. Overrides to the general policy for orders placed during a
   * specific set of time intervals.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverride seasonal_overrides = 14 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  int getSeasonalOverridesCount();

  /**
   *
   *
   * <pre>
   * Optional. Overrides to the general policy for orders placed during a
   * specific set of time intervals.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverride seasonal_overrides = 14 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  java.util.List<
          ? extends
              com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverrideOrBuilder>
      getSeasonalOverridesOrBuilderList();

  /**
   *
   *
   * <pre>
   * Optional. Overrides to the general policy for orders placed during a
   * specific set of time intervals.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverride seasonal_overrides = 14 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.SeasonalOverrideOrBuilder
      getSeasonalOverridesOrBuilder(int index);

  /**
   *
   *
   * <pre>
   * Optional. The restocking fee that applies to all return reason categories.
   * This would be treated as a free restocking fee if the value is not set.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.RestockingFee restocking_fee = 6 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return Whether the restockingFee field is set.
   */
  boolean hasRestockingFee();

  /**
   *
   *
   * <pre>
   * Optional. The restocking fee that applies to all return reason categories.
   * This would be treated as a free restocking fee if the value is not set.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.RestockingFee restocking_fee = 6 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The restockingFee.
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.RestockingFee getRestockingFee();

  /**
   *
   *
   * <pre>
   * Optional. The restocking fee that applies to all return reason categories.
   * This would be treated as a free restocking fee if the value is not set.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.RestockingFee restocking_fee = 6 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.RestockingFeeOrBuilder
      getRestockingFeeOrBuilder();

  /**
   *
   *
   * <pre>
   * Optional. The return methods of how customers can return an item. This
   * value is required to not be empty unless the type of return policy is
   * noReturns.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnMethod return_methods = 7 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return A list containing the returnMethods.
   */
  java.util.List<com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnMethod>
      getReturnMethodsList();

  /**
   *
   *
   * <pre>
   * Optional. The return methods of how customers can return an item. This
   * value is required to not be empty unless the type of return policy is
   * noReturns.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnMethod return_methods = 7 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The count of returnMethods.
   */
  int getReturnMethodsCount();

  /**
   *
   *
   * <pre>
   * Optional. The return methods of how customers can return an item. This
   * value is required to not be empty unless the type of return policy is
   * noReturns.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnMethod return_methods = 7 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @param index The index of the element to return.
   * @return The returnMethods at the given index.
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnMethod getReturnMethods(
      int index);

  /**
   *
   *
   * <pre>
   * Optional. The return methods of how customers can return an item. This
   * value is required to not be empty unless the type of return policy is
   * noReturns.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnMethod return_methods = 7 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return A list containing the enum numeric values on the wire for returnMethods.
   */
  java.util.List<java.lang.Integer> getReturnMethodsValueList();

  /**
   *
   *
   * <pre>
   * Optional. The return methods of how customers can return an item. This
   * value is required to not be empty unless the type of return policy is
   * noReturns.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnMethod return_methods = 7 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @param index The index of the value to return.
   * @return The enum numeric value on the wire of returnMethods at the given index.
   */
  int getReturnMethodsValue(int index);

  /**
   *
   *
   * <pre>
   * Optional. The item conditions accepted for returns must not be empty unless
   * the type of return policy is 'noReturns'.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ItemCondition item_conditions = 8 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return A list containing the itemConditions.
   */
  java.util.List<com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ItemCondition>
      getItemConditionsList();

  /**
   *
   *
   * <pre>
   * Optional. The item conditions accepted for returns must not be empty unless
   * the type of return policy is 'noReturns'.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ItemCondition item_conditions = 8 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The count of itemConditions.
   */
  int getItemConditionsCount();

  /**
   *
   *
   * <pre>
   * Optional. The item conditions accepted for returns must not be empty unless
   * the type of return policy is 'noReturns'.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ItemCondition item_conditions = 8 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @param index The index of the element to return.
   * @return The itemConditions at the given index.
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ItemCondition getItemConditions(
      int index);

  /**
   *
   *
   * <pre>
   * Optional. The item conditions accepted for returns must not be empty unless
   * the type of return policy is 'noReturns'.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ItemCondition item_conditions = 8 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return A list containing the enum numeric values on the wire for itemConditions.
   */
  java.util.List<java.lang.Integer> getItemConditionsValueList();

  /**
   *
   *
   * <pre>
   * Optional. The item conditions accepted for returns must not be empty unless
   * the type of return policy is 'noReturns'.
   * </pre>
   *
   * <code>
   * repeated .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ItemCondition item_conditions = 8 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @param index The index of the value to return.
   * @return The enum numeric value on the wire of itemConditions at the given index.
   */
  int getItemConditionsValue(int index);

  /**
   *
   *
   * <pre>
   * Optional. The return shipping fee. Should be set only when customer need to
   * download and print the return label.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnShippingFee return_shipping_fee = 9 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return Whether the returnShippingFee field is set.
   */
  boolean hasReturnShippingFee();

  /**
   *
   *
   * <pre>
   * Optional. The return shipping fee. Should be set only when customer need to
   * download and print the return label.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnShippingFee return_shipping_fee = 9 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The returnShippingFee.
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnShippingFee
      getReturnShippingFee();

  /**
   *
   *
   * <pre>
   * Optional. The return shipping fee. Should be set only when customer need to
   * download and print the return label.
   * </pre>
   *
   * <code>
   * .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnShippingFee return_shipping_fee = 9 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnShippingFeeOrBuilder
      getReturnShippingFeeOrBuilder();

  /**
   *
   *
   * <pre>
   * Required. The return policy uri. This can used by Google to do a sanity
   * check for the policy. It must be a valid URL.
   * </pre>
   *
   * <code>string return_policy_uri = 10 [(.google.api.field_behavior) = REQUIRED];</code>
   *
   * @return The returnPolicyUri.
   */
  java.lang.String getReturnPolicyUri();

  /**
   *
   *
   * <pre>
   * Required. The return policy uri. This can used by Google to do a sanity
   * check for the policy. It must be a valid URL.
   * </pre>
   *
   * <code>string return_policy_uri = 10 [(.google.api.field_behavior) = REQUIRED];</code>
   *
   * @return The bytes for returnPolicyUri.
   */
  com.google.protobuf.ByteString getReturnPolicyUriBytes();

  /**
   *
   *
   * <pre>
   * Optional. This field specifies if business
   * only accepts defective products for returns.
   * </pre>
   *
   * <code>optional bool accept_defective_only = 11 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return Whether the acceptDefectiveOnly field is set.
   */
  boolean hasAcceptDefectiveOnly();

  /**
   *
   *
   * <pre>
   * Optional. This field specifies if business
   * only accepts defective products for returns.
   * </pre>
   *
   * <code>optional bool accept_defective_only = 11 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The acceptDefectiveOnly.
   */
  boolean getAcceptDefectiveOnly();

  /**
   *
   *
   * <pre>
   * Optional. The field specifies the number of
   * days it takes for business to process refunds.
   * </pre>
   *
   * <code>optional int32 process_refund_days = 12 [(.google.api.field_behavior) = OPTIONAL];</code>
   *
   * @return Whether the processRefundDays field is set.
   */
  boolean hasProcessRefundDays();

  /**
   *
   *
   * <pre>
   * Optional. The field specifies the number of
   * days it takes for business to process refunds.
   * </pre>
   *
   * <code>optional int32 process_refund_days = 12 [(.google.api.field_behavior) = OPTIONAL];</code>
   *
   * @return The processRefundDays.
   */
  int getProcessRefundDays();

  /**
   *
   *
   * <pre>
   * Optional. This field specifies if business
   * allows customers to exchange products.
   * </pre>
   *
   * <code>optional bool accept_exchange = 13 [(.google.api.field_behavior) = OPTIONAL];</code>
   *
   * @return Whether the acceptExchange field is set.
   */
  boolean hasAcceptExchange();

  /**
   *
   *
   * <pre>
   * Optional. This field specifies if business
   * allows customers to exchange products.
   * </pre>
   *
   * <code>optional bool accept_exchange = 13 [(.google.api.field_behavior) = OPTIONAL];</code>
   *
   * @return The acceptExchange.
   */
  boolean getAcceptExchange();

  /**
   *
   *
   * <pre>
   * Optional. The field specifies the return
   * label source.
   * </pre>
   *
   * <code>
   * optional .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnLabelSource return_label_source = 15 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return Whether the returnLabelSource field is set.
   */
  boolean hasReturnLabelSource();

  /**
   *
   *
   * <pre>
   * Optional. The field specifies the return
   * label source.
   * </pre>
   *
   * <code>
   * optional .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnLabelSource return_label_source = 15 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The enum numeric value on the wire for returnLabelSource.
   */
  int getReturnLabelSourceValue();

  /**
   *
   *
   * <pre>
   * Optional. The field specifies the return
   * label source.
   * </pre>
   *
   * <code>
   * optional .google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnLabelSource return_label_source = 15 [(.google.api.field_behavior) = OPTIONAL];
   * </code>
   *
   * @return The returnLabelSource.
   */
  com.google.shopping.merchant.accounts.v1.OnlineReturnPolicy.ReturnLabelSource
      getReturnLabelSource();
}
