...
import com.google.shopping.merchant.accounts.v1.stub.GbpAccountsServiceStub;
 ...
 *      <td><p> Link the specified merchant to a GBP account for all countries.
 * <p>  To run this method, you must have admin access to the Merchant Center account. If you don't have admin access, the request fails with the error message `User is not an administrator of account {ACCOUNT_ID}`.</td>
 *      <td>
 *      <p>Request object method variants only take one parameter, a request object, which must be constructed before the call.</p>
 *      <ul>
...
public class GbpAccountsServiceClient implements BackgroundResource {
   ...
   *
   * <p>To run this method, you must have admin access to the Merchant Center account. If you don't
   * have admin access, the request fails with the error message `User is not an administrator of
   * account {ACCOUNT_ID}`.
   *
   * <p>Sample code:
   *
