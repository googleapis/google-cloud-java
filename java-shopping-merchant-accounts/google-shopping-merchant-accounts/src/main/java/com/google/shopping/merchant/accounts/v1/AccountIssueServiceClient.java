...
import com.google.shopping.merchant.accounts.v1.stub.AccountIssueServiceStub;
 ...
 *      <td><p> Lists all account issues of a Merchant Center account.
 * <p>  When called on a multi-client account, this method only returns issues belonging to that account, not its sub-accounts. To retrieve issues for sub-accounts, you must first call the [accounts.listSubaccounts][google.shopping.merchant.accounts.v1.AccountsService.ListSubAccounts] method to obtain a list of sub-accounts, and then call `accounts.issues.list` for each sub-account individually.</td>
 *      <td>
...
public class AccountIssueServiceClient implements BackgroundResource {
   ...
   *
   * <p>When called on a multi-client account, this method only returns issues belonging to that
   * account, not its sub-accounts. To retrieve issues for sub-accounts, you must first call the
   * [accounts.listSubaccounts][google.shopping.merchant.accounts.v1.AccountsService.ListSubAccounts]
   * method to obtain a list of sub-accounts, and then call `accounts.issues.list` for each
