...
public class CloudTasksClientTest {
  ...
  public static void startStaticServer() {
    ...
    mockServiceHelper =
        new MockServiceHelper(
            ...
            Arrays.<MockGrpcService>asList(mockCloudTasks, mockLocations));
  ...
  @Test
  public void uploadQueueYamlTest() throws Exception {
    Empty expectedResponse = Empty.newBuilder().build();
    mockCloudTasks.addResponse(expectedResponse);
    ...
    client.uploadQueueYaml(request);

    List<AbstractMessage> actualRequests = mockCloudTasks.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    UploadQueueYamlRequest actualRequest = ((UploadQueueYamlRequest) actualRequests.get(0));

    Assert.assertEquals(request.getAppId(), actualRequest.getAppId());
    Assert.assertEquals(request.getHttpBody(), actualRequest.getHttpBody());
    Assert.assertTrue(
        channelProvider.isHeaderSent(
            ApiClientHeaderProvider.getDefaultApiClientHeaderKey(),
            GaxGrpcProperties.getDefaultApiClientHeaderPattern()));
  ...
  @Test
  public void uploadQueueYamlExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockCloudTasks.addException(exception);
