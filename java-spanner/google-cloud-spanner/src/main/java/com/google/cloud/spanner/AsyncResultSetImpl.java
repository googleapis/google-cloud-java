...
class AsyncResultSetImpl extends ForwardingStructReader
  ...
  private void closeDelegateResultSet() {
    ...
    if (shouldClose) {
      try {
        ResultSet delegate = delegateResultSet.get();
        if (delegate != null) {
          delegate.close();
        }
      } catch (Throwable t) {
    ...
    public void run() {
      try {
        ...
        while (true) {
          ...
          if (hasNext) {
            boolean added = buffer.offer(delegateResultSet.get().getCurrentRowAsStruct());
            Preconditions.checkState(added, "Failed to buffer row despite available capacity");
            scheduleCallbackIfNecessary();
          } else {
            synchronized (monitor) {
              finished = true;
  ...
  private boolean isDataAvailable() {
    try {
      ...
      // Returning true ensures that any exception encountered while checking for available data or
      // initializing the delegate is not swallowed. The producer will proceed to invoke next(),
      // where the exception is caught, recorded in executionException, and propagated to the
      // caller.
      return true;
    }
  }
