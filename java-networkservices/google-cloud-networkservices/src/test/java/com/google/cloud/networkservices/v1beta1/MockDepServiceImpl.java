...
import com.google.cloud.networkservices.v1beta1.DepServiceGrpc.DepServiceImplBase;
...
public class MockDepServiceImpl extends DepServiceImplBase {
  ...
  public void listExtensionBindings(
      ...
      StreamObserver<ListExtensionBindingsResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof ListExtensionBindingsResponse) {
      requests.add(request);
      responseObserver.onNext(((ListExtensionBindingsResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method ListExtensionBindings, expected %s or"
                      + " %s",
                  response == null ? "null" : response.getClass().getName(),
                  ListExtensionBindingsResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
  ...
  public void getExtensionBinding(
      GetExtensionBindingRequest request, StreamObserver<ExtensionBinding> responseObserver) {
    Object response = responses.poll();
    if (response instanceof ExtensionBinding) {
      requests.add(request);
      responseObserver.onNext(((ExtensionBinding) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method GetExtensionBinding, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  ExtensionBinding.class.getName(),
                  Exception.class.getName())));
    }
  }
  ...
  public void createExtensionBinding(
      CreateExtensionBindingRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method CreateExtensionBinding, expected %s or"
                      + " %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
  ...
  public void updateExtensionBinding(
      UpdateExtensionBindingRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method UpdateExtensionBinding, expected %s or"
                      + " %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
  ...
  public void deleteExtensionBinding(
      DeleteExtensionBindingRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method DeleteExtensionBinding, expected %s or"
                      + " %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
