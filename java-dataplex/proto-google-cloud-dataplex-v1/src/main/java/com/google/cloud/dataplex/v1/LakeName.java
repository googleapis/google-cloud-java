...
public class LakeName implements ResourceName {
  private static final PathTemplate PROJECT_LOCATION_LAKE =
      PathTemplate.createWithoutUrlEncoding("projects/{project}/locations/{location}/lakes/{lake}");
  ...
  public static LakeName of(String project, String location, String lake) {
    return newBuilder().setProject(project).setLocation(location).setLake(lake).build();
  }

  public static String format(String project, String location, String lake) {
    return newBuilder().setProject(project).setLocation(location).setLake(lake).build().toString();
  }

  public static @Nullable LakeName parse(String formattedString) {
    if (formattedString.isEmpty()) {
      return null;
    }
    Map<String, String> matchMap =
        PROJECT_LOCATION_LAKE.validatedMatch(
            formattedString, "LakeName.parse: formattedString not in valid format");
    return of(matchMap.get("project"), matchMap.get("location"), matchMap.get("lake"));
  }

  public static List<LakeName> parseList(List<String> formattedStrings) {
    List<LakeName> list = new ArrayList<>(formattedStrings.size());
  ...
  public static List<String> toStringList(List<@Nullable LakeName> values) {
  ...
  public String toString() {
    return PROJECT_LOCATION_LAKE.instantiate(
