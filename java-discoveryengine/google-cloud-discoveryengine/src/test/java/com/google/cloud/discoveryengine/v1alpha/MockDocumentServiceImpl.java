...
import com.google.cloud.discoveryengine.v1alpha.DocumentServiceGrpc.DocumentServiceImplBase;
...
public class MockDocumentServiceImpl extends DocumentServiceImplBase {
  ...
  public void purgeDocuments(
      PurgeDocumentsRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method PurgeDocuments, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
    }
