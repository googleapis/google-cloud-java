...
get_gapic_opts() {
  ...
  if [ "${generate_version_java}" == "true" ]; then
    generate_version_java_opt="generate-version-java"
  else
    generate_version_java_opt=""
  fi
  ...
  echo "transport=${transport},${rest_numeric_enums},${generate_version_java_opt},grpc-service-config=${service_config},gapic-config=${gapic_yaml},api-service-config=${service_yaml},repo=${repo},artifact=${artifact}"
