...
class ResumableUploadTransportStubComposerTest {
  ...
  void generate_grpcStub_withCredentials_initializesHttpStub() {
    ...
    // The constructor guards on credentials and hands its own context and settings to the upload
    // stub; deriving the HTTP/JSON ClientContext is the upload stub's job, not this stub's.
    assertThat(code).contains("if (clientContext.getCredentials() != null) {");
    assertThat(code).contains("this.resumableUploadStub =");
    assertThat(code)
        .contains(
            "HttpJsonResumableUploadServiceResumableUploadStub.createFrom(clientContext,"
                + " settings);");
    assertThat(code).doesNotContain("ManagedHttpJsonChannel");
    assertThat(code).doesNotContain("HttpJsonTransportChannel");
    assertThat(code).doesNotContain("HttpJsonCallContext");
    assertThat(code).doesNotContain("uploadHeaders");
  }
