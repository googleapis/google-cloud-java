...
class HttpJsonServiceResumableUploadStubClassComposerTest {
  ...
  private static String generateStubCode() {
  ...
  @Test
  void generate_emitsMethodDescriptorWithUploadPathPrefix() {
    String code = generateStubCode();

    // Verify key descriptor imports
    ...
    assertThat(code).contains("uploadMediaStartUploadMethodDescriptor =");
    assertThat(code)
        .contains(
    ...
    assertThat(code).contains("methodDescriptors.add(uploadMediaStartUploadMethodDescriptor);");
  }

  @Test
  void generate_emitsCallableGetter() {
    String code = generateStubCode();
  ...
  @Test
  void generate_implementsBackgroundResource() {
    String code = generateStubCode();
  ...
  @Test
  void generate_emitsCreateFromFactoryDerivingHttpJsonClientContext() {
    String code = generateStubCode();

    assertThat(code)
        .contains("public static HttpJsonResumableUploadServiceResumableUploadStub createFrom(");

    // The context's endpoint wins over the settings': on the create(ClientContext) entry point the
    // settings are pure defaults and all real configuration lives in the context.
    assertThat(code)
        .contains(
            "clientContext.getEndpoint() != null ? clientContext.getEndpoint() :"
                + " settings.getEndpoint();");

    // A dedicated HTTP/JSON channel is stood up rather than reusing the caller's transport, and
    // registered as a background resource so it is closed with the enclosing client.
    assertThat(code).contains("ManagedHttpJsonChannel.newBuilder()");
    assertThat(code)
        .contains("HttpJsonTransportChannel.create(httpJsonManagedChannel);");
    assertThat(code).contains(".setBackgroundResources(Arrays.asList(httpJsonTransportChannel))");

    // Internal headers are rebuilt for HTTP/JSON; the caller's external headers take precedence.
    assertThat(code)
        .contains(
            "ResumableUploadServiceStubSettings.defaultHttpJsonApiClientHeaderProviderBuilder()");
    assertThat(code).contains("uploadHeaders.putAll(clientContext.getHeaders());");

    // Credentials, executor and clock are inherited from the given context.
    ...
    assertThat(code).contains(".setClock(clientContext.getClock())");
    assertThat(code).contains("return create(httpJsonClientContext, settings);");
  }
