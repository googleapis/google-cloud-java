...
class ResumableUploadSettingsComposerTest {
  ...
  void generateServiceStubSettings_resumableUploadCallSettings() {
    ...
    assertThat(code)
        .contains("Note that the settings configured here apply strictly to the initial session initiation");
    assertThat(code).contains("request, and not to the subsequent data transfer requests.");
  }
