 ...
 * {@code backgroundResources} member: {@code close}, {@code shutdown}, {@code isShutdown}, {@code
 * isTerminated}, {@code shutdownNow} and {@code awaitTermination}.
 *
 * <p>This logic is shared by {@link AbstractTransportServiceStubClassComposer} and by the resumable
 * upload stub composer. The resumable upload stub is not a transport service stub -- it has no
 * callable factory, no settings-driven callables and no operations stub -- so it cannot extend
 * {@link AbstractTransportServiceStubClassComposer}, but the lifecycle plumbing is identical and is
 * reused from here rather than duplicated.
 */
@NullMarked
