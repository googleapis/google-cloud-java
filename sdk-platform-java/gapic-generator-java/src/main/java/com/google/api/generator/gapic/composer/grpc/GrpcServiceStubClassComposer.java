...
public class GrpcServiceStubClassComposer extends AbstractTransportServiceStubClassComposer {
  ...
  protected List<Statement> createResumableUploadStubInitStatements(
    ...
    // Resumable uploads execute over HTTP/JSON and require credentials, which cannot be extracted
    // from a pre-constructed gRPC Channel -- hence the null guard. The upload stub owns the
    // derivation of its own HTTP/JSON ClientContext (see
    // HttpJson<Service>ResumableUploadStub.createFrom), so all this stub does is hand over its
    // context and settings.
    TypeNode uploadStubType =
        typeStore.get(String.format(RESUMABLE_UPLOAD_STUB_PATTERN, service.name()));
    Expr getCredentialsExpr =
        MethodInvocationExpr.builder()
            .setExprReferenceExpr(clientContextVarExpr)
            .setMethodName("getCredentials")
            .setReturnType(TypeNode.OBJECT)
            .build();
    VariableExpr assignTargetVarExpr =
        resumableUploadStubVarExpr.toBuilder().setExprReferenceExpr(thisExpr).build();
    Statement assignResumableUploadStub =
        ExprStatement.withExpr(
            AssignmentExpr.builder()
                .setVariableExpr(assignTargetVarExpr)
                .setValueExpr(
                    MethodInvocationExpr.builder()
                        .setStaticReferenceType(uploadStubType)
                        .setMethodName("createFrom")
        ...
        IfStatement.builder()
            .setConditionExpr(
                RelationalOperationExpr.notEqualToWithExprs(
                    getCredentialsExpr, ValueExpr.createNullExpr()))
            .setBody(Arrays.asList(assignResumableUploadStub))
