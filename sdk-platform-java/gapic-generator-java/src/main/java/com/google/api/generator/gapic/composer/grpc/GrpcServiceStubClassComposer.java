...
public class GrpcServiceStubClassComposer extends AbstractTransportServiceStubClassComposer {
  ...
  protected List<Statement> createResumableUploadStubBackgroundResourceStatements(
    ...
    // Credentials cannot be extracted from a pre-constructed gRPC Channel, hence the null guard.
    // The upload stub derives its own HTTP/JSON ClientContext from the one passed here.
    Expr getCredentialsExpr =
        MethodInvocationExpr.builder()
            ...
            .build();
    List<Statement> uploadStubPresentStmts =
        ImmutableList.<Statement>builder()
            .addAll(resumableUploadStubInitStatements)
            .add(uploadStubPresentBackgroundResourceStmt)
            .build();
    List<Statement> uploadStubAbsentStmts =
        Arrays.asList(
            ExprStatement.withExpr(
                AssignmentExpr.builder()
                    .setVariableExpr(thisResumableUploadStubVarExpr)
    ...
    return Collections.singletonList(
        IfStatement.builder()
            ...
            .setBody(uploadStubPresentStmts)
            .setElseBody(uploadStubAbsentStmts)
            .build());
  }
