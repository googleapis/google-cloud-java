...
import com.google.cloud.aiplatform.v1beta1.IndexEndpointServiceGrpc.IndexEndpointServiceImplBase;
...
public class MockIndexEndpointServiceImpl extends IndexEndpointServiceImplBase {
  private List<AbstractMessage> requests;
  private Queue<Object> responses;

  public MockIndexEndpointServiceImpl() {
  ...
  public void updateIndexEndpoint(
      UpdateIndexEndpointRequest request, StreamObserver<IndexEndpoint> responseObserver) {
    Object response = responses.poll();
    if (response instanceof IndexEndpoint) {
      requests.add(request);
      responseObserver.onNext(((IndexEndpoint) response));
  ...
  public void mutateDeployedIndex(
      MutateDeployedIndexRequest request, StreamObserver<Operation> responseObserver) {
    Object response = responses.poll();
    if (response instanceof Operation) {
      requests.add(request);
      responseObserver.onNext(((Operation) response));
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method MutateDeployedIndex, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  Operation.class.getName(),
                  Exception.class.getName())));
    }
