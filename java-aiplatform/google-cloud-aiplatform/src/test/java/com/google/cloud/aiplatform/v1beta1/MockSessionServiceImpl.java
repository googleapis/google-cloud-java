...
import com.google.cloud.aiplatform.v1beta1.SessionServiceGrpc.SessionServiceImplBase;
...
public class MockSessionServiceImpl extends SessionServiceImplBase {
  ...
  public void appendEvent(
      AppendEventRequest request, StreamObserver<AppendEventResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof AppendEventResponse) {
      requests.add(request);
      responseObserver.onNext(((AppendEventResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method AppendEvent, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  AppendEventResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
}
