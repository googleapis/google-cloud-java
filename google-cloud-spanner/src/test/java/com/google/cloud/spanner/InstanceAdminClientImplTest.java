...
public class InstanceAdminClientImplTest {
  ...
  public void getInstanceConfig() {
    when(rpc.getInstanceConfig(CONFIG_NAME)).thenReturn(
        InstanceConfig.newBuilder().setName(CONFIG_NAME).build());
  ...
  public void listInstanceConfigs() {
    ...
    when(rpc.listInstanceConfigs(1, null)).thenReturn(
        new Paginated<InstanceConfig>(
            ImmutableList.of(InstanceConfig.newBuilder().setName(CONFIG_NAME).build()), nextToken));
    when(rpc.listInstanceConfigs(1, nextToken)).thenReturn(
        new Paginated<InstanceConfig>(
            ImmutableList.of(InstanceConfig.newBuilder().setName(CONFIG_NAME2).build()), ""));
    List<com.google.cloud.spanner.InstanceConfig> configs = Lists.newArrayList(
        client.listInstanceConfigs(Options.pageSize(1)).iterateAll());
  ...
  public void createInstance() {
    when(rpc.createInstance("projects/" + PROJECT_ID, INSTANCE_ID, getInstanceProto())).thenReturn(
        ...
        .build());
    Operation<Instance, CreateInstanceMetadata> op = client.createInstance(
  ...
  public void updateInstanceMetadata() {
    ...
    .thenReturn(com.google.longrunning.Operation.newBuilder()
        ...
        .build());
    InstanceInfo instanceInfo = InstanceInfo.newBuilder(InstanceId.of(INSTANCE_NAME))
      .setInstanceConfigId(InstanceConfigId.of(CONFIG_NAME))
      .setNodeCount(2)
      .build();
    Operation<Instance, UpdateInstanceMetadata> op = client.updateInstance(instanceInfo,
