...
public class LLRBBlackValueNode<K, V> extends LLRBValueNode<K, V> {
     ...
     * size will still have an amortized constant runtime. The first time size() may have to
     * traverse the entire tree. However, the red black tree algorithm guarantees that every red
     * node has two black children. So future invocations of the size() function will have to go at
     * most 2 levels deep if the child is a red node.
     *
