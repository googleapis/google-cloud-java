...
/**
 * This is an array backed implementation of ImmutableSortedMap. It uses arrays and linear lookups to achieve good
 * memory efficiency while maintaining good performance for small collections. To avoid degrading performance with
 * increasing collection size it will automatically convert to a RBTreeSortedMap after an insert call above a
 * certain threshold.
 */
public class ArraySortedMap<K, V> extends ImmutableSortedMap<K, V> {
    ...
    public static <K, V> ArraySortedMap<K, V> fromMap(Map<K, V> map, Comparator<K> comparator) {
        return buildFrom(new ArrayList<K>(map.keySet()), map, Builder.<K>identityTranslator(), comparator);
    }
    ...
    /**
     * This does a linear scan which is simpler than a binary search. For a small collection size this still
     * should be as fast a as binary search.
     */
