...
public class HttpJsonResumableUploadServiceResumableUploadStub implements BackgroundResource {
  ...
  /**
   * Constructs an instance from the given client context. An HTTP/JSON context is used as is. A
   * context built for any other transport is replaced by one carrying a dedicated HTTP/JSON
   * transport for the upload, which is registered as a background resource and closed with the
   * caller.
   */
  public static HttpJsonResumableUploadServiceResumableUploadStub create(
    ...
    if (clientContext.getTransportChannel() instanceof HttpJsonTransportChannel) {
      return new HttpJsonResumableUploadServiceResumableUploadStub(clientContext, settings);
    }
    return new HttpJsonResumableUploadServiceResumableUploadStub(
        deriveHttpJsonClientContext(clientContext, settings), settings);
  }

  private static ClientContext deriveHttpJsonClientContext(
      ClientContext clientContext, ResumableUploadServiceStubSettings settings) {
        ...
        new HashMap<>(
            ApiClientHeaderProvider.newBuilder()
                .setGeneratedLibToken(
                    "gapic",
                    GaxProperties.getLibraryVersion(ResumableUploadServiceStubSettings.class))
                .setTransportToken(
                    GaxHttpJsonProperties.getHttpJsonTokenName(),
                    GaxHttpJsonProperties.getHttpJsonVersion())
                .setQuotaProjectIdToken(clientContext.getQuotaProjectId())
                .build()
                .getHeaders());
  ...
  private HttpJsonResumableUploadServiceResumableUploadStub(
      ClientContext clientContext, ResumableUploadServiceStubSettings settings) {
