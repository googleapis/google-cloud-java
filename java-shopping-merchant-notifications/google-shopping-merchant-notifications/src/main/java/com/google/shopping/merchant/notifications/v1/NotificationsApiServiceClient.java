...
import com.google.shopping.merchant.notifications.v1.stub.NotificationsApiServiceStub;
 ...
 *      <td><p> Creates a notification subscription for a business. For standalone or subaccounts accounts, the business can create a subscription for self. For MCAs, the business can create a subscription for all managed accounts or for a specific subaccount.
 * <p>  We will allow the following types of notification subscriptions to exist together (per business as a subscriber per event type):
 * <p>  1. Subscription for all managed accounts + subscription for self.
 * <p>  2. Multiple "partial" subscriptions for managed accounts + subscription for self.
 * <p>  we will not allow (per business as a subscriber per event type):
 * <p>  1. Multiple self subscriptions.
 * <p>  2. Multiple "all managed accounts" subscriptions.
 * <p>  3. "All managed accounts" subscription and partial subscriptions at the same time.
 * <p>  4. Multiple partial subscriptions for the same target account.</td>
 *      <td>
 *      <p>Request object method variants only take one parameter, a request object, which must be constructed before the call.</p>
 *      <ul>
 ...
 *    </tr>
 *    <tr>
 *      <td><p> GetNotificationSubscriptionHealthMetrics</td>
 *      <td><p> Retrieves notification subscription health metrics for a merchant. We recommend calling this periodically (hourly or daily) to monitor health of your callback endpoint.</td>
 *      <td>
 *      <p>Request object method variants only take one parameter, a request object, which must be constructed before the call.</p>
 *      <ul>
 *           <li><p> getNotificationSubscriptionHealthMetrics(GetNotificationSubscriptionHealthMetricsRequest request)
 *      </ul>
 *      <p>"Flattened" method variants have converted the fields of the request object into function parameters to enable multiple ways to call the same method.</p>
 *      <ul>
 *           <li><p> getNotificationSubscriptionHealthMetrics(NotificationSubscriptionHealthMetricsName name)
 *           <li><p> getNotificationSubscriptionHealthMetrics(String name)
 *      </ul>
 *      <p>Callable method variants take no parameters and return an immutable API callable object, which can be used to initiate calls to the service.</p>
 *      <ul>
 *           <li><p> getNotificationSubscriptionHealthMetricsCallable()
 *      </ul>
 *       </td>
 *    </tr>
 *  </table>
...
public class NotificationsApiServiceClient implements BackgroundResource {
   ...
   * for all managed accounts or for a specific subaccount.
   *
   * <p>We will allow the following types of notification subscriptions to exist together (per
   * business as a subscriber per event type):
   *
   * <p>1. Subscription for all managed accounts + subscription for self.
   *
   * <p>2. Multiple "partial" subscriptions for managed accounts + subscription for self.
   ...
   * <p>3. "All managed accounts" subscription and partial subscriptions at the same time.
   *
   * <p>4. Multiple partial subscriptions for the same target account.
   *
   * <p>Sample code:
   *
   * <pre>{@code
  ...
  // AUTO-GENERATED DOCUMENTATION AND METHOD.
  /**
   * Retrieves notification subscription health metrics for a merchant. We recommend calling this
   * periodically (hourly or daily) to monitor health of your callback endpoint.
   *
   * <p>Sample code:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param name Required. The `name` of the notification subscription for which metrics are
   *     retrieved. Format:
  ...
  public final NotificationSubscriptionHealthMetrics getNotificationSubscriptionHealthMetrics(
    ...
    GetNotificationSubscriptionHealthMetricsRequest request =
        GetNotificationSubscriptionHealthMetricsRequest.newBuilder()
            .setName(name == null ? null : name.toString())
            .build();
    return getNotificationSubscriptionHealthMetrics(request);
  }
  ...
  public final NotificationSubscriptionHealthMetrics getNotificationSubscriptionHealthMetrics(
      String name) {
    GetNotificationSubscriptionHealthMetricsRequest request =
        GetNotificationSubscriptionHealthMetricsRequest.newBuilder().setName(name).build();
    return getNotificationSubscriptionHealthMetrics(request);
  }
  ...
  public final NotificationSubscriptionHealthMetrics getNotificationSubscriptionHealthMetrics(
      GetNotificationSubscriptionHealthMetricsRequest request) {
    return getNotificationSubscriptionHealthMetricsCallable().call(request);
  }
   ...
   *                   .toString())
   *           .build();
   *   ApiFuture<NotificationSubscriptionHealthMetrics> future =
   *       notificationsApiServiceClient
