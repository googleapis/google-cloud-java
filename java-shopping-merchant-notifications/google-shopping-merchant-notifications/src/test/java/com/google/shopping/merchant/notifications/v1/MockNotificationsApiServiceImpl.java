...
public class MockNotificationsApiServiceImpl extends NotificationsApiServiceImplBase {
  ...
  public void getNotificationSubscriptionHealthMetrics(
      ...
      StreamObserver<NotificationSubscriptionHealthMetrics> responseObserver) {
    Object response = responses.poll();
    if (response instanceof NotificationSubscriptionHealthMetrics) {
      requests.add(request);
      responseObserver.onNext(((NotificationSubscriptionHealthMetrics) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
          ...
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method"
                      + " GetNotificationSubscriptionHealthMetrics, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  NotificationSubscriptionHealthMetrics.class.getName(),
                  Exception.class.getName())));
    }
  }
}
